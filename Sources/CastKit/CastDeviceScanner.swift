...
extension CastDevice {
  convenience init?(service: NetService, info: [String: String]) {
    ...
    if let address = service.addresses?.first {
      ipAddress = address.withUnsafeBytes { (rawBuffer: UnsafeRawBufferPointer) -> String? in
        guard let baseAddress = rawBuffer.baseAddress else { return nil }
        let pointer = baseAddress.assumingMemoryBound(to: sockaddr.self)
        var hostName = [CChar](repeating: 0, count: Int(NI_MAXHOST))

        return getnameinfo(pointer, socklen_t(address.count), &hostName, socklen_t(NI_MAXHOST), nil, 0, NI_NUMERICHOST) == 0 ? String(cString: hostName) : nil
      }
    }
...
public final class CastDeviceScanner: NSObject, @unchecked Sendable {
